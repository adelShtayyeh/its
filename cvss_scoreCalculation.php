...
// }
//-----------------------------------------------------------------------------------
$AS = "SELECT DISTINCT browser,`version`,platform FROM agentstrings WHERE checkCVSS = 0";
...
if ($result_AS->num_rows > 0) {
    while ($rowAS = $result_AS->fetch_assoc()) {
        ...
        $sqlupdate = "UPDATE agentstrings SET cvss_score=" . $totalCVE . ", time_cvss_score='" . $time_cvss_score . "', checkCVSS = 1 WHERE browser LIKE '" . $rowAS['browser'] .
    ...
    $sqlupdate = "UPDATE agentstrings SET time_cvss_score='" . $time_cvss_score . "' WHERE 1";
    $conn->query($sqlupdate);
